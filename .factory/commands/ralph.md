...
2. **Identify** - List files to touch before editing (use `glob` to find them)
3. **Execute** - Make edits with clear explanations (use `write` or `edit` tools)
4. **Verify** - Run tests or static checks, inspect diffs (use `bash` for tests)
...
- `bash` - Execute shell commands (for tests, linting, etc.)
...
- **Tool efficiency**: use `grep` and `glob` to explore before making changes
